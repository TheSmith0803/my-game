...
Array_List *array_list_create(usize item_size, usize initial_capacity) {
    ...
    if (!list) {
        ERROR_RETURN(NULL, "Could not allocate memory for Array_List code 1\n");
    }
    ...
    if (!list->items) {
        ERROR_RETURN(NULL, "Could not allocate memory for Array_List code 2\n"); 
    }
...
usize array_list_append(Array_List *list, const void *item) {
    if (list->len == list->capacity) {
        ...
        if (!items) {
            ERROR_RETURN(-1, "Could not allocate memory for Array_List code 3\n");
        }
...
u8 array_list_remove(Array_List *list, usize index) {
    ...
    if (index >= list->len) {
        ERROR_RETURN(1, "Index is out of bounds\n");
    }
    if (list->len == 1) {
        list->len = 0;
        return 0;
